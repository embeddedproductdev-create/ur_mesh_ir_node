[
    {
        "id": "9c54f3c8c089fa45",
        "type": "tab",
        "label": "IR_GWY",
        "disabled": false,
        "info": ""
    },
    {
        "id": "cbc87edddeafa6e2",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "204713403aa79b73",
            "a1d471c7a1ac6d19",
            "4784fa14e5f61222",
            "712b0dd90ac477ac"
        ],
        "x": 964,
        "y": 979,
        "w": 302,
        "h": 122
    },
    {
        "id": "ed17d2bd59277ec4",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "5ce3249b75ce8fe1",
            "4fecaab8343e81b2",
            "36359e27780e1101",
            "1552c5129f19d08c"
        ],
        "x": 964,
        "y": 839,
        "w": 302,
        "h": 122
    },
    {
        "id": "52561f8f00799893",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "40b827d6816a3171",
            "b7b98c5720e48a59",
            "cfe578b1ac763857",
            "122f647a3278988e",
            "072867c7f8ac00b7",
            "b65fbe19bc6ee32e",
            "fdce0da76334e6cd"
        ],
        "x": 634,
        "y": 439,
        "w": 302,
        "h": 242
    },
    {
        "id": "77dbd83bc15edd0d",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "b31831e2ebdd5389",
            "837db81f06121660",
            "51d2a2894dce54dc",
            "40a8ee475f22d9e2",
            "7c7fb17ab8f7ae3c",
            "20b3b596f0475a22"
        ],
        "x": 964,
        "y": 1119,
        "w": 302,
        "h": 122
    },
    {
        "id": "7a8449c9ea1a73f4",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "9b3a45e93d286387",
            "fcbd2d266fda1677",
            "cc4b3926bd6acf48",
            "c539b4b9e686af06",
            "0a1b3f59f2772da8",
            "43bdd9183e22c1c2",
            "6ebcae263ab95141"
        ],
        "x": 964,
        "y": 439,
        "w": 302,
        "h": 242
    },
    {
        "id": "de24a92e56605186",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "61a21b0dfc329c1b",
            "471d2bb250bd8591",
            "a88d935d0e84888b",
            "f8a246f18472e35a"
        ],
        "x": 964,
        "y": 699,
        "w": 302,
        "h": 122
    },
    {
        "id": "3cd359cda2495a9f",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "d535b79268bbe6d4",
            "29de8361f796d0eb"
        ],
        "x": 634,
        "y": 699,
        "w": 302,
        "h": 82
    },
    {
        "id": "a669af41e81ab495",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "399648122e87bc79",
            "29ae6e74d7a9bd23",
            "286aa5e15bc90728",
            "2d5f779104f92712"
        ],
        "x": 964,
        "y": 1259,
        "w": 302,
        "h": 122
    },
    {
        "id": "e543388b507d5126",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "6ab8e03f940bba8b",
            "bddb4bd4c8e94474",
            "c43445b06f58a6ac",
            "38c6e51186b314d2",
            "a2e72fc4f574a532"
        ],
        "x": 158,
        "y": 813,
        "w": 784,
        "h": 1014
    },
    {
        "id": "de0691412ac8cf3b",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "name": "Heartbeat Publish Configuration",
        "style": {
            "label": true,
            "label-position": "n",
            "color": "#ff0000"
        },
        "nodes": [
            "1ccd405f3db422ef",
            "322ada899a109eb8",
            "ee22631dbc83c928",
            "8bcebb234ea8e372",
            "2dc9e3aa5c32b20b",
            "7bec6a19cc8496b2",
            "cafaa35d688f77c2",
            "befcee3405925bec",
            "19f9d8acae892860"
        ],
        "x": 154,
        "y": 79,
        "w": 562,
        "h": 122
    },
    {
        "id": "9ddd9c0c4e81fd9b",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "3c1f73dbd3a3272d",
            "2d440ac677f9e945",
            "8f9c2342f993e093",
            "3cbbe6ac0c62b902",
            "0842edbeb22cad01",
            "252c5748d089f2bb",
            "c089c9b06c418c77",
            "b8443472257e1a17",
            "6ef18bba4bf092ba",
            "432d5faf1bda02a5"
        ],
        "x": 884,
        "y": 219,
        "w": 492,
        "h": 202
    },
    {
        "id": "cb5cb5d3935e7873",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "51d756f0287c566a",
            "25dd451ebd1479ed",
            "c67579603d7368f5",
            "e164c72d5f12d03a",
            "fec9c17deae6082b"
        ],
        "x": 1284,
        "y": 439,
        "w": 272,
        "h": 203
    },
    {
        "id": "93250921e6d76682",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "ab9419de7e778cb0",
            "4feae91ab5ce8c5c",
            "191a94da1e8180b7",
            "4467f98c8c390b23",
            "23bfba6a76a35f09",
            "0ffc627b8817dc3f",
            "e3a9faf0e181a981",
            "4bdffcde7ad70484",
            "c0023466461eafc7",
            "333b9ea3fbd2051a",
            "fa71f37e4b477f9e",
            "b692e8d986fbe3ca",
            "32f40b81ed0016f8",
            "2f852626ea4f3d20",
            "6b97eb34e2a509b0",
            "4eb1fc758a9ae0ab",
            "8859639b97d711e2"
        ],
        "x": 154,
        "y": 439,
        "w": 452,
        "h": 362
    },
    {
        "id": "2607943a8d78aacb",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "b13a1bb3b94904e5",
            "b841e632f05a3d46"
        ],
        "x": 874,
        "y": 79,
        "w": 252,
        "h": 82
    },
    {
        "id": "15bb5fa223da72c7",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "name": "Sending section",
        "style": {
            "label": true
        },
        "nodes": [
            "1dc6314952a6d669",
            "616de90333ed9553",
            "2ef46d99a4ebdfc5",
            "ba803b0afe363686",
            "053ce1b0d0d3214d",
            "5a2988701c22d1b7",
            "b34cdd7048ec6797",
            "685d2a8af0eb6b32",
            "ac1edac598491f3a",
            "03d8bca961e516f9",
            "1e825eb98754dcc6",
            "a1834883d3b51665",
            "30f9d0ca4699e562",
            "8a12f284de75a643",
            "e33e844ed5bfa869"
        ],
        "x": 154,
        "y": 179,
        "w": 342,
        "h": 242
    },
    {
        "id": "8453c3c42afc8ad4",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "name": "Receiving section",
        "style": {
            "label": true
        },
        "nodes": [
            "e228c4d490914989",
            "7cdc66cb731a791b",
            "83308769e0176daa",
            "97cf2c580f97d3a8",
            "8a2a0c44e1e64c1f",
            "3a2e111abd6339ad",
            "ecec0caadc2cc8e4",
            "39f2bf2cc6e3cb64",
            "c1593d5e1ceb29bf",
            "c08974d04257dbf2",
            "b609244a96d993da",
            "5a0355fe38be07c8",
            "751e1553c639b344",
            "e13960097ca55baa",
            "6be6ca25b09c9e73",
            "5cfe864119145b75",
            "c8164a9ebca09224",
            "a3943b3ace12ead9",
            "06c8087ea218b976",
            "23ad1d166b2b7e88"
        ],
        "x": 514,
        "y": 219,
        "w": 302,
        "h": 202
    },
    {
        "id": "9478a12968e0865e",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "dbb315e0d2b26759",
            "682989149c9de46c",
            "c6e19313557c32b3"
        ],
        "x": 964,
        "y": 1399,
        "w": 282,
        "h": 122
    },
    {
        "id": "0e9f32d208ba4b2e",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "3fc78c676c50424d",
            "c6e0cf1e0aa4a4d6",
            "1a77cd19feb8ef4e"
        ],
        "x": 1294,
        "y": 699,
        "w": 262,
        "h": 122
    },
    {
        "id": "88753f5d31542be8",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "style": {
            "stroke": "#999999",
            "stroke-opacity": "1",
            "fill": "none",
            "fill-opacity": "1",
            "label": true,
            "label-position": "nw",
            "color": "#a4a4a4"
        },
        "nodes": [
            "25e29728a45fcd87",
            "1b29124113a3f7aa",
            "70f0814af507b85d"
        ],
        "x": 1294,
        "y": 839,
        "w": 262,
        "h": 122
    },
    {
        "id": "6ab8e03f940bba8b",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "g": "e543388b507d5126",
        "name": "Temperature",
        "style": {
            "label": true,
            "label-position": "n",
            "color": "#ff0000"
        },
        "nodes": [
            "cc2ab1f2355064b1",
            "24e7b92ecb969e06",
            "2f62d9215e830238",
            "659ef7d66c2b8b07",
            "4bca7ed0f514e807",
            "e6e1ce7ce767e504",
            "b66a805bda4cfbfa",
            "5f8024103d968087",
            "efc24025befe12eb",
            "e403bd0c71b35dd7",
            "d06bc4a882a2aec5",
            "e8dd3f8db2399256",
            "67b1dee2c42688ad",
            "ee7ea02e8a377e9e",
            "eab4239ce0d31092",
            "6ddaca7b2e4b018e",
            "502ede3dd7559b10"
        ],
        "x": 184,
        "y": 839,
        "w": 732,
        "h": 162
    },
    {
        "id": "bddb4bd4c8e94474",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "g": "e543388b507d5126",
        "name": "TempLockLowLimit",
        "style": {
            "label": true,
            "label-position": "n",
            "color": "#ff0000"
        },
        "nodes": [
            "f1661c066b65a4e7",
            "3f405d96e7bfa0ef",
            "d219c7a9829673c7",
            "14bc40a8a2e699be",
            "b1169c41b465ef65",
            "8226a735523b6e98",
            "a0bf4b6779a2ae3f",
            "3fc668b56fd473a9",
            "108a167e208a2eb7",
            "6ed4144ffcf1f763",
            "2ba0b2a271860dac",
            "db5a236f7ed231f0",
            "fe8b29c763f15711",
            "07565358ace50897",
            "26f4ba5dc97758c1",
            "8cb2c8a6ac59714a",
            "1d93cfe13f3deb04"
        ],
        "x": 184,
        "y": 1039,
        "w": 732,
        "h": 162
    },
    {
        "id": "c43445b06f58a6ac",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "g": "e543388b507d5126",
        "name": "TempLockUpLimit",
        "style": {
            "label": true,
            "label-position": "n",
            "color": "#ff0000"
        },
        "nodes": [
            "7e3a3016f83d2494",
            "5c6e09da4620390c",
            "9eccf0d85e245e6e",
            "58c5b0920bd45afb",
            "c6eaca59d65c8649",
            "190acef13b46dbd1",
            "03442eccd36a4f50",
            "7d3fa7bfbf77789d",
            "d8649b4aa6a4bddd",
            "2a2bb2291b3e38ff",
            "bb2308a51d55283f",
            "ffe9a6122c0fc5a5",
            "4dfbd51d344e2c14",
            "e65d2a58378f1448",
            "712dcc15a861b5d9",
            "ab1edee97ec23dac",
            "f932dd4a2d51ff77"
        ],
        "x": 184,
        "y": 1239,
        "w": 732,
        "h": 162
    },
    {
        "id": "38c6e51186b314d2",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "g": "e543388b507d5126",
        "name": "Starting Temperature",
        "style": {
            "label": true,
            "label-position": "n",
            "color": "#ff0000"
        },
        "nodes": [
            "4124febb2ccdbec4",
            "eb334519f509740d",
            "0d877d3ba67432ad",
            "4c8c8a7866ec297b",
            "db596926b6473797",
            "b19fc4fc23e254a7",
            "70e5f5d1fbdebafe",
            "eb8fdb9366c0cbfd",
            "751d9a7ce447ef95",
            "812388750930990d",
            "f6ae080f498b5ae1",
            "1905b3e90ec791e9",
            "d31aed671d48287a",
            "7b2003f18d98edb2",
            "395ac3fa2c7726a0",
            "149ef0b6108c288e"
        ],
        "x": 184,
        "y": 1439,
        "w": 732,
        "h": 162
    },
    {
        "id": "a2e72fc4f574a532",
        "type": "group",
        "z": "9c54f3c8c089fa45",
        "g": "e543388b507d5126",
        "name": "Ending Temperature",
        "style": {
            "label": true,
            "label-position": "n",
            "color": "#ff0000"
        },
        "nodes": [
            "a0ea49b49acea139",
            "a62f660515e074e0",
            "f81cbaaad3b34539",
            "66d3e34b31a12241",
            "2c01ae7b2748d4be",
            "01e345e30338f447",
            "614cda551775c11d",
            "92e219519ab93f1e",
            "710f21a15ba926aa",
            "5c6ceb71bc82058e",
            "d910d67059cd97e9",
            "3b1150787b4e9215",
            "6a4e980fe59862c2",
            "b7cfe130556c401d",
            "9a9ff2bc08a39941",
            "629ddef91f242a15"
        ],
        "x": 184,
        "y": 1639,
        "w": 732,
        "h": 162
    },
    {
        "id": "204713403aa79b73",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "cbc87edddeafa6e2",
        "name": "SwingV",
        "func": "if(flow.get(\"swingvcount\")>0){\n    flow.set(\"SwingV\", msg.payload);\n    return msg\n}\nelse{\n    flow.set(\"swingvcount\",1)\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1185,
        "y": 1040,
        "wires": [
            [
                "712b0dd90ac477ac"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "a1d471c7a1ac6d19",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "cbc87edddeafa6e2",
        "name": "Swing V ON",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1",
        "payloadType": "num",
        "x": 1080,
        "y": 1020,
        "wires": [
            [
                "204713403aa79b73",
                "712b0dd90ac477ac"
            ]
        ]
    },
    {
        "id": "4784fa14e5f61222",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "cbc87edddeafa6e2",
        "name": "Swing V OFF",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "0",
        "payloadType": "num",
        "x": 1080,
        "y": 1060,
        "wires": [
            [
                "204713403aa79b73",
                "712b0dd90ac477ac"
            ]
        ]
    },
    {
        "id": "712b0dd90ac477ac",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "cbc87edddeafa6e2",
        "name": "link out 4",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 1225,
        "y": 1040,
        "wires": []
    },
    {
        "id": "5ce3249b75ce8fe1",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "ed17d2bd59277ec4",
        "name": "SwingH",
        "func": "if(flow.get(\"swinghcount\")>0){\n    flow.set(\"SwingH\", msg.payload);\n    return msg\n}\nelse{\n    flow.set(\"swinghcount\",1);\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1185,
        "y": 900,
        "wires": [
            [
                "1552c5129f19d08c"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "4fecaab8343e81b2",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "ed17d2bd59277ec4",
        "name": "Swing H ON",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1",
        "payloadType": "num",
        "x": 1080,
        "y": 880,
        "wires": [
            [
                "5ce3249b75ce8fe1",
                "1552c5129f19d08c"
            ]
        ]
    },
    {
        "id": "36359e27780e1101",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "ed17d2bd59277ec4",
        "name": "Swing H OFF",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "0",
        "payloadType": "num",
        "x": 1080,
        "y": 920,
        "wires": [
            [
                "5ce3249b75ce8fe1",
                "1552c5129f19d08c"
            ]
        ]
    },
    {
        "id": "1552c5129f19d08c",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "ed17d2bd59277ec4",
        "name": "link out 5",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 1225,
        "y": 900,
        "wires": []
    },
    {
        "id": "40b827d6816a3171",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "52561f8f00799893",
        "name": "Mode",
        "func": "if(flow.get(\"modecount\")>0){\n    flow.set(\"Mode\", msg.payload);\n    return msg;\n}\nelse {\n    flow.set(\"modecount\", 1);\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 855,
        "y": 560,
        "wires": [
            [
                "fdce0da76334e6cd"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "b7b98c5720e48a59",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "52561f8f00799893",
        "name": "Cool",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "Cool",
        "payloadType": "str",
        "x": 730,
        "y": 480,
        "wires": [
            [
                "40b827d6816a3171",
                "fdce0da76334e6cd"
            ]
        ]
    },
    {
        "id": "cfe578b1ac763857",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "52561f8f00799893",
        "name": "Hot",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "Hot",
        "payloadType": "str",
        "x": 730,
        "y": 520,
        "wires": [
            [
                "40b827d6816a3171",
                "fdce0da76334e6cd"
            ]
        ]
    },
    {
        "id": "122f647a3278988e",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "52561f8f00799893",
        "name": "Dry",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "Dry",
        "payloadType": "str",
        "x": 730,
        "y": 560,
        "wires": [
            [
                "40b827d6816a3171",
                "fdce0da76334e6cd"
            ]
        ]
    },
    {
        "id": "072867c7f8ac00b7",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "52561f8f00799893",
        "name": "Fan",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "Fan",
        "payloadType": "str",
        "x": 730,
        "y": 640,
        "wires": [
            [
                "40b827d6816a3171",
                "fdce0da76334e6cd"
            ]
        ]
    },
    {
        "id": "b65fbe19bc6ee32e",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "52561f8f00799893",
        "name": "Auto",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "Auto",
        "payloadType": "str",
        "x": 730,
        "y": 600,
        "wires": [
            [
                "40b827d6816a3171",
                "fdce0da76334e6cd"
            ]
        ]
    },
    {
        "id": "fdce0da76334e6cd",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "52561f8f00799893",
        "name": "link out 1",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 895,
        "y": 560,
        "wires": []
    },
    {
        "id": "b31831e2ebdd5389",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "77dbd83bc15edd0d",
        "name": "OnTimer",
        "func": "if(flow.get(\"ontimercount\")>0){\n    flow.set(\"OnTimer\", msg.payload);\n    return msg\n}\nelse {\n    flow.set(\"ontimercount\",1)\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1165,
        "y": 1160,
        "wires": [
            [
                "20b3b596f0475a22"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "837db81f06121660",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "77dbd83bc15edd0d",
        "name": "OffTimer",
        "func": "if(flow.get(\"offtimercount\")>0){\n    flow.set(\"OffTimer\", msg.payload);\n    return msg\n}\nelse {\n    flow.set(\"offtimercount\",1)\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1165,
        "y": 1200,
        "wires": [
            [
                "7c7fb17ab8f7ae3c"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "51d2a2894dce54dc",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "77dbd83bc15edd0d",
        "name": "On Timer",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "0",
        "payloadType": "num",
        "x": 1070,
        "y": 1160,
        "wires": [
            [
                "b31831e2ebdd5389",
                "20b3b596f0475a22"
            ]
        ]
    },
    {
        "id": "40a8ee475f22d9e2",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "77dbd83bc15edd0d",
        "name": "Off Timer",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "0",
        "payloadType": "num",
        "x": 1070,
        "y": 1200,
        "wires": [
            [
                "837db81f06121660",
                "7c7fb17ab8f7ae3c"
            ]
        ]
    },
    {
        "id": "7c7fb17ab8f7ae3c",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "77dbd83bc15edd0d",
        "name": "link out 3",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 1225,
        "y": 1200,
        "wires": []
    },
    {
        "id": "20b3b596f0475a22",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "77dbd83bc15edd0d",
        "name": "link out 12",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 1225,
        "y": 1160,
        "wires": []
    },
    {
        "id": "9b3a45e93d286387",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "7a8449c9ea1a73f4",
        "name": "FanSpeed",
        "func": "if(flow.get(\"fancount\")>0){\n    flow.set(\"FanSpeed\", msg.payload);\n    return msg\n}\nelse {\n    flow.set(\"fancount\",1)\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1185,
        "y": 560,
        "wires": [
            [
                "6ebcae263ab95141"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "fcbd2d266fda1677",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "7a8449c9ea1a73f4",
        "name": "Fan 1",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1",
        "payloadType": "num",
        "x": 1060,
        "y": 480,
        "wires": [
            [
                "9b3a45e93d286387",
                "6ebcae263ab95141"
            ]
        ]
    },
    {
        "id": "cc4b3926bd6acf48",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "7a8449c9ea1a73f4",
        "name": "Fan 2",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "2",
        "payloadType": "num",
        "x": 1060,
        "y": 520,
        "wires": [
            [
                "9b3a45e93d286387",
                "6ebcae263ab95141"
            ]
        ]
    },
    {
        "id": "c539b4b9e686af06",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "7a8449c9ea1a73f4",
        "name": "Fan 3",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "3",
        "payloadType": "num",
        "x": 1060,
        "y": 560,
        "wires": [
            [
                "9b3a45e93d286387",
                "6ebcae263ab95141"
            ]
        ]
    },
    {
        "id": "0a1b3f59f2772da8",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "7a8449c9ea1a73f4",
        "name": "Fan 4",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "4",
        "payloadType": "num",
        "x": 1060,
        "y": 600,
        "wires": [
            [
                "9b3a45e93d286387",
                "6ebcae263ab95141"
            ]
        ]
    },
    {
        "id": "43bdd9183e22c1c2",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "7a8449c9ea1a73f4",
        "name": "Fan 5",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "5",
        "payloadType": "num",
        "x": 1060,
        "y": 640,
        "wires": [
            [
                "9b3a45e93d286387",
                "6ebcae263ab95141"
            ]
        ]
    },
    {
        "id": "6ebcae263ab95141",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "7a8449c9ea1a73f4",
        "name": "link out 10",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 1225,
        "y": 560,
        "wires": []
    },
    {
        "id": "61a21b0dfc329c1b",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "de24a92e56605186",
        "name": "Power",
        "func": "if(flow.get(\"powercount\")>0){\n    flow.set(\"Power\", msg.payload);\n    return msg\n}\nelse {\n    flow.set(\"powercount\",1);\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1185,
        "y": 760,
        "wires": [
            [
                "f8a246f18472e35a"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "471d2bb250bd8591",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "de24a92e56605186",
        "name": "Power ON",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1",
        "payloadType": "num",
        "x": 1080,
        "y": 740,
        "wires": [
            [
                "61a21b0dfc329c1b",
                "f8a246f18472e35a"
            ]
        ]
    },
    {
        "id": "a88d935d0e84888b",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "de24a92e56605186",
        "name": "Power OFF",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "0",
        "payloadType": "num",
        "x": 1080,
        "y": 780,
        "wires": [
            [
                "61a21b0dfc329c1b",
                "f8a246f18472e35a"
            ]
        ]
    },
    {
        "id": "f8a246f18472e35a",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "de24a92e56605186",
        "name": "link out 9",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 1225,
        "y": 760,
        "wires": []
    },
    {
        "id": "d535b79268bbe6d4",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "3cd359cda2495a9f",
        "name": "Location",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "2nd floor",
        "payloadType": "str",
        "x": 740,
        "y": 740,
        "wires": [
            [
                "29de8361f796d0eb"
            ]
        ]
    },
    {
        "id": "29de8361f796d0eb",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "3cd359cda2495a9f",
        "name": "Location",
        "func": "flow.set(\"Location\", msg.payload);",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 895,
        "y": 740,
        "wires": [
            []
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "399648122e87bc79",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "a669af41e81ab495",
        "name": "Locking",
        "func": "if(flow.get(\"lockingcount\")>0){\n    flow.set(\"Locking\", msg.payload);\n    return msg\n}\nelse {\n    flow.set(\"lockingcount\",1)\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1175,
        "y": 1320,
        "wires": [
            [
                "2d5f779104f92712"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "29ae6e74d7a9bd23",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a669af41e81ab495",
        "name": "Locking ON",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1",
        "payloadType": "num",
        "x": 1080,
        "y": 1300,
        "wires": [
            [
                "399648122e87bc79",
                "2d5f779104f92712"
            ]
        ]
    },
    {
        "id": "286aa5e15bc90728",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a669af41e81ab495",
        "name": "Locking OFF",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "0",
        "payloadType": "num",
        "x": 1080,
        "y": 1340,
        "wires": [
            [
                "399648122e87bc79",
                "2d5f779104f92712"
            ]
        ]
    },
    {
        "id": "2d5f779104f92712",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "a669af41e81ab495",
        "name": "link out 11",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 1225,
        "y": 1320,
        "wires": []
    },
    {
        "id": "cc2ab1f2355064b1",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "Temperature",
        "func": "if(flow.get(\"tempcount\")>0){\nflow.set(\"Temperature\", msg.payload);\nreturn msg\n}\nelse{\n    flow.set(\"tempcount\",1)\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 835,
        "y": 920,
        "wires": [
            [
                "502ede3dd7559b10"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "24e7b92ecb969e06",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "18",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "18",
        "payloadType": "num",
        "x": 280,
        "y": 880,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "2f62d9215e830238",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "19",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "19",
        "payloadType": "num",
        "x": 400,
        "y": 880,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "659ef7d66c2b8b07",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "20",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "20",
        "payloadType": "num",
        "x": 520,
        "y": 880,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "4bca7ed0f514e807",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "21",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "21",
        "payloadType": "num",
        "x": 640,
        "y": 880,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "e6e1ce7ce767e504",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "22",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "22",
        "payloadType": "num",
        "x": 760,
        "y": 880,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "b66a805bda4cfbfa",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "23",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "23",
        "payloadType": "num",
        "x": 280,
        "y": 920,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "5f8024103d968087",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "24",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "24",
        "payloadType": "num",
        "x": 400,
        "y": 920,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "efc24025befe12eb",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "25",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "25",
        "payloadType": "num",
        "x": 520,
        "y": 920,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "e403bd0c71b35dd7",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "26",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "26",
        "payloadType": "num",
        "x": 640,
        "y": 920,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "d06bc4a882a2aec5",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "27",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "27",
        "payloadType": "num",
        "x": 760,
        "y": 920,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "e8dd3f8db2399256",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "28",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "28",
        "payloadType": "num",
        "x": 280,
        "y": 960,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "67b1dee2c42688ad",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "29",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "29",
        "payloadType": "num",
        "x": 400,
        "y": 960,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "ee7ea02e8a377e9e",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "30",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "30",
        "payloadType": "num",
        "x": 520,
        "y": 960,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "eab4239ce0d31092",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "31",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "31",
        "payloadType": "num",
        "x": 640,
        "y": 960,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "6ddaca7b2e4b018e",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "32",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "32",
        "payloadType": "num",
        "x": 760,
        "y": 960,
        "wires": [
            [
                "cc2ab1f2355064b1",
                "502ede3dd7559b10"
            ]
        ]
    },
    {
        "id": "502ede3dd7559b10",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "6ab8e03f940bba8b",
        "name": "link out 6",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 875,
        "y": 920,
        "wires": []
    },
    {
        "id": "f1661c066b65a4e7",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "TempLockLowLimit",
        "func": "if(flow.get(\"templocklowcount\")>0){\nflow.set(\"TempLockLowLimit\", msg.payload);\nreturn msg\n}\nelse {\n    flow.set(\"templocklowcount\",1)\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 835,
        "y": 1120,
        "wires": [
            [
                "1d93cfe13f3deb04"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "3f405d96e7bfa0ef",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "18",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "18",
        "payloadType": "num",
        "x": 280,
        "y": 1080,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "d219c7a9829673c7",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "19",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "19",
        "payloadType": "num",
        "x": 400,
        "y": 1080,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "14bc40a8a2e699be",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "20",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "20",
        "payloadType": "num",
        "x": 520,
        "y": 1080,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "b1169c41b465ef65",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "21",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "21",
        "payloadType": "num",
        "x": 640,
        "y": 1080,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "8226a735523b6e98",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "22",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "22",
        "payloadType": "num",
        "x": 760,
        "y": 1080,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "a0bf4b6779a2ae3f",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "23",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "23",
        "payloadType": "num",
        "x": 280,
        "y": 1120,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "3fc668b56fd473a9",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "24",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "24",
        "payloadType": "num",
        "x": 400,
        "y": 1120,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "108a167e208a2eb7",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "25",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "25",
        "payloadType": "num",
        "x": 520,
        "y": 1120,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "6ed4144ffcf1f763",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "26",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "26",
        "payloadType": "num",
        "x": 640,
        "y": 1120,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "2ba0b2a271860dac",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "27",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "27",
        "payloadType": "num",
        "x": 760,
        "y": 1120,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "db5a236f7ed231f0",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "28",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "28",
        "payloadType": "num",
        "x": 280,
        "y": 1160,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "fe8b29c763f15711",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "29",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "29",
        "payloadType": "num",
        "x": 400,
        "y": 1160,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "07565358ace50897",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "30",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "30",
        "payloadType": "num",
        "x": 520,
        "y": 1160,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "26f4ba5dc97758c1",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "31",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "31",
        "payloadType": "num",
        "x": 640,
        "y": 1160,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "8cb2c8a6ac59714a",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "32",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "32",
        "payloadType": "num",
        "x": 760,
        "y": 1160,
        "wires": [
            [
                "f1661c066b65a4e7",
                "1d93cfe13f3deb04"
            ]
        ]
    },
    {
        "id": "1d93cfe13f3deb04",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "bddb4bd4c8e94474",
        "name": "link out 7",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 875,
        "y": 1120,
        "wires": []
    },
    {
        "id": "7e3a3016f83d2494",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "TempLockUpLimit",
        "func": "if(flow.get(\"templockupcount\")>0){\nflow.set(\"TempLockUpLimit\", msg.payload);\nreturn msg\n}\nelse{\n    flow.set(\"templockupcount\",1)\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 835,
        "y": 1320,
        "wires": [
            [
                "f932dd4a2d51ff77"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "5c6e09da4620390c",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "18",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "18",
        "payloadType": "num",
        "x": 280,
        "y": 1280,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "9eccf0d85e245e6e",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "19",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "19",
        "payloadType": "num",
        "x": 400,
        "y": 1280,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "58c5b0920bd45afb",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "20",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "20",
        "payloadType": "num",
        "x": 520,
        "y": 1280,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "c6eaca59d65c8649",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "21",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "21",
        "payloadType": "num",
        "x": 640,
        "y": 1280,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "190acef13b46dbd1",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "22",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "22",
        "payloadType": "num",
        "x": 760,
        "y": 1280,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "03442eccd36a4f50",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "23",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "23",
        "payloadType": "num",
        "x": 280,
        "y": 1320,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "7d3fa7bfbf77789d",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "24",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "24",
        "payloadType": "num",
        "x": 400,
        "y": 1320,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "d8649b4aa6a4bddd",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "25",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "25",
        "payloadType": "num",
        "x": 520,
        "y": 1320,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "2a2bb2291b3e38ff",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "26",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "26",
        "payloadType": "num",
        "x": 640,
        "y": 1320,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "bb2308a51d55283f",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "27",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "27",
        "payloadType": "num",
        "x": 760,
        "y": 1320,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "ffe9a6122c0fc5a5",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "28",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "28",
        "payloadType": "num",
        "x": 280,
        "y": 1360,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "4dfbd51d344e2c14",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "29",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "29",
        "payloadType": "num",
        "x": 400,
        "y": 1360,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "e65d2a58378f1448",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "30",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "30",
        "payloadType": "num",
        "x": 520,
        "y": 1360,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "712dcc15a861b5d9",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "31",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "31",
        "payloadType": "num",
        "x": 640,
        "y": 1360,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "ab1edee97ec23dac",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "32",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "32",
        "payloadType": "num",
        "x": 760,
        "y": 1360,
        "wires": [
            [
                "7e3a3016f83d2494",
                "f932dd4a2d51ff77"
            ]
        ]
    },
    {
        "id": "f932dd4a2d51ff77",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "c43445b06f58a6ac",
        "name": "link out 8",
        "mode": "link",
        "links": [
            "1e825eb98754dcc6"
        ],
        "x": 875,
        "y": 1320,
        "wires": []
    },
    {
        "id": "1ccd405f3db422ef",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "de0691412ac8cf3b",
        "name": "10",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "10",
        "payloadType": "num",
        "x": 250,
        "y": 140,
        "wires": [
            [
                "befcee3405925bec"
            ]
        ]
    },
    {
        "id": "322ada899a109eb8",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "de0691412ac8cf3b",
        "name": "600",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "600",
        "payloadType": "num",
        "x": 330,
        "y": 120,
        "wires": [
            [
                "befcee3405925bec"
            ]
        ]
    },
    {
        "id": "ee22631dbc83c928",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "de0691412ac8cf3b",
        "name": "900",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "900",
        "payloadType": "num",
        "x": 430,
        "y": 120,
        "wires": [
            [
                "befcee3405925bec"
            ]
        ]
    },
    {
        "id": "8bcebb234ea8e372",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "de0691412ac8cf3b",
        "name": "1200",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1200",
        "payloadType": "num",
        "x": 530,
        "y": 120,
        "wires": [
            [
                "befcee3405925bec"
            ]
        ]
    },
    {
        "id": "2dc9e3aa5c32b20b",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "de0691412ac8cf3b",
        "name": "1500",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1500",
        "payloadType": "num",
        "x": 330,
        "y": 160,
        "wires": [
            [
                "befcee3405925bec"
            ]
        ]
    },
    {
        "id": "7bec6a19cc8496b2",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "de0691412ac8cf3b",
        "name": "1800",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1800",
        "payloadType": "num",
        "x": 430,
        "y": 160,
        "wires": [
            [
                "befcee3405925bec"
            ]
        ]
    },
    {
        "id": "cafaa35d688f77c2",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "de0691412ac8cf3b",
        "name": "2100",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "2100",
        "payloadType": "num",
        "x": 530,
        "y": 160,
        "wires": [
            [
                "befcee3405925bec"
            ]
        ]
    },
    {
        "id": "befcee3405925bec",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "de0691412ac8cf3b",
        "name": "GwyPublishPeriod",
        "func": "flow.set(\"GwyPublishPeriod\", msg.payload);\nflow.set(\"NodePublishPeriod\",msg.payload);\nif (flow.get(\"Device\") == 1) {\n    node.warn(\"Currently Active device : \" + flow.get(\"NodeSerNo\"));\n    msg.payload = 107;\n    return msg;\n}\nelse {\n    node.warn(\"Currently Active device : \" + flow.get(\"GwySerNo\"));\n    msg.payload = 7;\n    return msg;\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 625,
        "y": 140,
        "wires": [
            [
                "19f9d8acae892860"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "19f9d8acae892860",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "de0691412ac8cf3b",
        "name": "link out 27",
        "mode": "link",
        "links": [
            "30f9d0ca4699e562"
        ],
        "x": 675,
        "y": 140,
        "wires": []
    },
    {
        "id": "3c1f73dbd3a3272d",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "9ddd9c0c4e81fd9b",
        "name": "ElementAddr",
        "func": "flow.set(\"ElementAddr\", msg.payload);",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1335,
        "y": 320,
        "wires": [
            []
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "2d440ac677f9e945",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "9ddd9c0c4e81fd9b",
        "name": "Node",
        "func": "flow.set(\"ac_control\", 103);\nflow.set(\"Device\",1);\nswitch(msg.payload)\n{\n    case 1:\n        flow.set(\"MacId\",\"\");\n        flow.set(\"NodeSerNo\", \"N00001\");\n        break;\n    case 2:\n        flow.set(\"MacId\",\"DC:DA:0C:74:BE:76\");\n        flow.set(\"NodeSerNo\", \"N00002\");\n        break;\n    case 3:\n        flow.set(\"MacId\",\"DC:DA:0C:74:BE:AE\");\n        flow.set(\"NodeSerNo\", \"N00003\");\n        break;\n    case 4:\n        flow.set(\"MacId\",\"DC:DA:0C:74:BE:9A\");\n        flow.set(\"NodeSerNo\", \"N00004\");\n        break;\n}\nflow.set(\"pub_conf\",107);",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1075,
        "y": 320,
        "wires": [
            []
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "8f9c2342f993e093",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "9ddd9c0c4e81fd9b",
        "name": "ElementAddr 5",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "5",
        "payloadType": "num",
        "x": 1210,
        "y": 260,
        "wires": [
            [
                "3c1f73dbd3a3272d"
            ]
        ]
    },
    {
        "id": "3cbbe6ac0c62b902",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "9ddd9c0c4e81fd9b",
        "name": "Node 2",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "2",
        "payloadType": "num",
        "x": 980,
        "y": 300,
        "wires": [
            [
                "2d440ac677f9e945"
            ]
        ]
    },
    {
        "id": "0842edbeb22cad01",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "9ddd9c0c4e81fd9b",
        "name": "Node 3",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "3",
        "payloadType": "num",
        "x": 980,
        "y": 340,
        "wires": [
            [
                "2d440ac677f9e945"
            ]
        ]
    },
    {
        "id": "252c5748d089f2bb",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "9ddd9c0c4e81fd9b",
        "name": "Node 1",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1",
        "payloadType": "num",
        "x": 980,
        "y": 260,
        "wires": [
            [
                "2d440ac677f9e945"
            ]
        ]
    },
    {
        "id": "c089c9b06c418c77",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "9ddd9c0c4e81fd9b",
        "name": "ElementAddr 6",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "6",
        "payloadType": "num",
        "x": 1210,
        "y": 300,
        "wires": [
            [
                "3c1f73dbd3a3272d"
            ]
        ]
    },
    {
        "id": "b8443472257e1a17",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "9ddd9c0c4e81fd9b",
        "name": "ElementAddr 7",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "7",
        "payloadType": "num",
        "x": 1210,
        "y": 340,
        "wires": [
            [
                "3c1f73dbd3a3272d"
            ]
        ]
    },
    {
        "id": "51d756f0287c566a",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "cb5cb5d3935e7873",
        "name": "GwySerNo",
        "func": "flow.set(\"GwySerNo\", msg.payload);\nflow.set(\"ac_control\",3);\nflow.set(\"pub_conf\",7);\nflow.set(\"Device\",0);",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1515,
        "y": 540,
        "wires": [
            []
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "25dd451ebd1479ed",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "cb5cb5d3935e7873",
        "name": "GWY00002",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "GWY00002",
        "payloadType": "str",
        "x": 1400,
        "y": 520,
        "wires": [
            [
                "51d756f0287c566a"
            ]
        ]
    },
    {
        "id": "c67579603d7368f5",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "cb5cb5d3935e7873",
        "name": "GWY00001",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "GWY00001",
        "payloadType": "str",
        "x": 1400,
        "y": 480,
        "wires": [
            [
                "51d756f0287c566a"
            ]
        ]
    },
    {
        "id": "e164c72d5f12d03a",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "cb5cb5d3935e7873",
        "name": "GWY00003",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "GWY00003",
        "payloadType": "str",
        "x": 1400,
        "y": 560,
        "wires": [
            [
                "51d756f0287c566a"
            ]
        ]
    },
    {
        "id": "ab9419de7e778cb0",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "Debug",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "10",
        "payloadType": "num",
        "x": 250,
        "y": 760,
        "wires": [
            [
                "6b97eb34e2a509b0"
            ]
        ]
    },
    {
        "id": "4feae91ab5ce8c5c",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "Registration",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "0",
        "payloadType": "num",
        "x": 270,
        "y": 480,
        "wires": [
            [
                "c0023466461eafc7"
            ]
        ]
    },
    {
        "id": "191a94da1e8180b7",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "Unregistration/Unprovisioning",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "2",
        "payloadType": "num",
        "x": 320,
        "y": 520,
        "wires": [
            [
                "333b9ea3fbd2051a"
            ]
        ]
    },
    {
        "id": "4467f98c8c390b23",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "AC Control",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "3",
        "payloadType": "num",
        "x": 260,
        "y": 560,
        "wires": [
            [
                "fa71f37e4b477f9e"
            ]
        ]
    },
    {
        "id": "23bfba6a76a35f09",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "AC Remote Reconfiguration",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "5",
        "payloadType": "num",
        "x": 320,
        "y": 600,
        "wires": [
            [
                "b692e8d986fbe3ca"
            ]
        ]
    },
    {
        "id": "0ffc627b8817dc3f",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "Heartbeat Publish Configuration",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "7",
        "payloadType": "num",
        "x": 330,
        "y": 640,
        "wires": [
            [
                "32f40b81ed0016f8"
            ]
        ]
    },
    {
        "id": "e3a9faf0e181a981",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "Teaching Mode",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "8",
        "payloadType": "num",
        "x": 280,
        "y": 680,
        "wires": [
            [
                "2f852626ea4f3d20"
            ]
        ]
    },
    {
        "id": "4bdffcde7ad70484",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "link out 25",
        "mode": "link",
        "links": [
            "a1834883d3b51665"
        ],
        "x": 565,
        "y": 620,
        "wires": []
    },
    {
        "id": "c0023466461eafc7",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "GwyPublishPeriod",
        "func": "msg.payload = 0;\nreturn msg;\n\n",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 495,
        "y": 480,
        "wires": [
            [
                "4bdffcde7ad70484"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "333b9ea3fbd2051a",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "GwyPublishPeriod",
        "func": "if(flow.get(\"Device\")==1)\n{\n    msg.payload = 102;\n    return msg;\n}\nelse\n{\n    msg.payload = 2;\n    return msg;\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 495,
        "y": 520,
        "wires": [
            [
                "4bdffcde7ad70484"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "fa71f37e4b477f9e",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "GwyPublishPeriod",
        "func": "if(flow.get(\"Device\")==1)\n{\n    msg.payload = 103;\n    return msg;\n}\nelse\n{\n    msg.payload = 3;\n    return msg;\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 495,
        "y": 560,
        "wires": [
            [
                "4bdffcde7ad70484"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "b692e8d986fbe3ca",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "GwyPublishPeriod",
        "func": "if(flow.get(\"Device\")==1)\n{\n    msg.payload = 105;\n    return msg;\n}\nelse\n{\n    msg.payload = 5;\n    return msg;\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 495,
        "y": 600,
        "wires": [
            [
                "4bdffcde7ad70484"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "32f40b81ed0016f8",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "GwyPublishPeriod",
        "func": "if(flow.get(\"Device\")==1)\n{\n    msg.payload = 107;\n    return msg;\n}\nelse\n{\n    msg.payload = 7;\n    return msg;\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 495,
        "y": 640,
        "wires": [
            [
                "4bdffcde7ad70484"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "2f852626ea4f3d20",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "GwyPublishPeriod",
        "func": "if(flow.get(\"Device\")==1)\n{\n    msg.payload = 108;\n    return msg;\n}\nelse\n{\n    msg.payload = 8;\n    return msg;\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 495,
        "y": 680,
        "wires": [
            [
                "4bdffcde7ad70484"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "6b97eb34e2a509b0",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "GwyPublishPeriod",
        "func": "if(flow.get(\"Device\")==1)\n{\n    msg.payload = 110;\n    return msg;\n}\nelse\n{\n    msg.payload = 10;\n    return msg;\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 495,
        "y": 760,
        "wires": [
            [
                "4bdffcde7ad70484"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "b13a1bb3b94904e5",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "2607943a8d78aacb",
        "name": "Usage",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "{\"General Instructions reg the flow\":{\"1)\":\"The boxes that have a tiny '1' marked on them means, they will automatically be injected once the flow is loaded. This is set a few default values\",\"2)\":\"This flow uses the same inject buttons for Gwy related control and Node related control, to make the flow small and less messy\",\"3)\":\"Since, the same inject buttons are used, the user will need to inject the respective Gwy/Node buttons and then inject the respective operation\",\"4)\":\"AC Control Packet can be sent out not just by injecting 'AC control packet' button, but also by injecting any AC Control related buttons, This is done to mimic the user operation with actual AC remote\"},\"Preliminary steps to be followed before using the flow\":{\"STEP 1\":\"Choose which Gwy you are using\",\"STEP 2\":\"Choose which Node you are using\",\"STEP 3\":\"Choose the right Element Address (If you already have a provisioned node)\"},\"Getting the ACKs from the right Gwy\":{\"1)\":\"Please check which debug box is 'activated' and which debug box is 'deactivated'\",\"2)\":\"If you are using GWY00003, then make sure only GWY00003 is activated and other two are deactivated to avoid getting too many ACKs\"}}",
        "payloadType": "json",
        "x": 970,
        "y": 120,
        "wires": [
            [
                "b841e632f05a3d46"
            ]
        ]
    },
    {
        "id": "b841e632f05a3d46",
        "type": "debug",
        "z": "9c54f3c8c089fa45",
        "g": "2607943a8d78aacb",
        "name": "debug 1",
        "active": true,
        "tosidebar": true,
        "console": false,
        "tostatus": false,
        "complete": "false",
        "statusVal": "",
        "statusType": "auto",
        "x": 1065,
        "y": 120,
        "wires": [],
        "l": false
    },
    {
        "id": "6ef18bba4bf092ba",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "9ddd9c0c4e81fd9b",
        "name": "Node 4",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "4",
        "payloadType": "num",
        "x": 980,
        "y": 380,
        "wires": [
            [
                "2d440ac677f9e945"
            ]
        ]
    },
    {
        "id": "432d5faf1bda02a5",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "9ddd9c0c4e81fd9b",
        "name": "ElementAddr 8",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "8",
        "payloadType": "num",
        "x": 1210,
        "y": 380,
        "wires": [
            [
                "3c1f73dbd3a3272d"
            ]
        ]
    },
    {
        "id": "1dc6314952a6d669",
        "type": "mqtt out",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "GWY00001/commands",
        "topic": "GWY00001/commands",
        "qos": "2",
        "retain": "false",
        "respTopic": "",
        "contentType": "",
        "userProps": "",
        "correl": "",
        "expiry": "",
        "broker": "f0e2d1c0.92bb3",
        "x": 405,
        "y": 260,
        "wires": [],
        "l": false
    },
    {
        "id": "616de90333ed9553",
        "type": "mqtt out",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "GWY00002/command",
        "topic": "GWY00002/command",
        "qos": "2",
        "retain": "false",
        "respTopic": "",
        "contentType": "",
        "userProps": "",
        "correl": "",
        "expiry": "",
        "broker": "dc9c4c6e985986ab",
        "x": 455,
        "y": 300,
        "wires": [],
        "l": false
    },
    {
        "id": "2ef46d99a4ebdfc5",
        "type": "mqtt out",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "GWY00003/command",
        "topic": "GWY00003/command",
        "qos": "2",
        "retain": "false",
        "respTopic": "",
        "contentType": "",
        "userProps": "",
        "correl": "",
        "expiry": "",
        "broker": "dc9c4c6e985986ab",
        "x": 405,
        "y": 340,
        "wires": [],
        "l": false
    },
    {
        "id": "ba803b0afe363686",
        "type": "switch",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "Gwy Req Router",
        "property": "GwySerNo",
        "propertyType": "flow",
        "rules": [
            {
                "t": "cont",
                "v": "GWY00001",
                "vt": "str"
            },
            {
                "t": "cont",
                "v": "GWY00002",
                "vt": "str"
            },
            {
                "t": "cont",
                "v": "GWY00003",
                "vt": "str"
            },
            {
                "t": "cont",
                "v": "GWY00004",
                "vt": "str"
            }
        ],
        "checkall": "true",
        "repair": false,
        "outputs": 4,
        "x": 355,
        "y": 320,
        "wires": [
            [
                "1dc6314952a6d669",
                "053ce1b0d0d3214d"
            ],
            [
                "616de90333ed9553",
                "5a2988701c22d1b7"
            ],
            [
                "2ef46d99a4ebdfc5",
                "b34cdd7048ec6797"
            ],
            [
                "685d2a8af0eb6b32",
                "ac1edac598491f3a"
            ]
        ],
        "l": false
    },
    {
        "id": "053ce1b0d0d3214d",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "GWY00001_CMD",
        "mode": "link",
        "links": [
            "751e1553c639b344"
        ],
        "x": 455,
        "y": 260,
        "wires": []
    },
    {
        "id": "5a2988701c22d1b7",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "GWY00002_CMD",
        "mode": "link",
        "links": [
            "e13960097ca55baa"
        ],
        "x": 455,
        "y": 220,
        "wires": []
    },
    {
        "id": "b34cdd7048ec6797",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "GWY00003_CMD",
        "mode": "link",
        "links": [
            "6be6ca25b09c9e73"
        ],
        "x": 455,
        "y": 340,
        "wires": []
    },
    {
        "id": "685d2a8af0eb6b32",
        "type": "link out",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "GWY00004_CMD",
        "mode": "link",
        "links": [
            "5cfe864119145b75"
        ],
        "x": 455,
        "y": 380,
        "wires": []
    },
    {
        "id": "ac1edac598491f3a",
        "type": "mqtt out",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "GWY00004/command",
        "topic": "GWY00004/command",
        "qos": "2",
        "retain": "false",
        "respTopic": "",
        "contentType": "",
        "userProps": "",
        "correl": "",
        "expiry": "",
        "broker": "dc9c4c6e985986ab",
        "x": 405,
        "y": 380,
        "wires": [],
        "l": false
    },
    {
        "id": "e228c4d490914989",
        "type": "mqtt in",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00001/messages",
        "topic": "GWY00001/messages",
        "qos": "2",
        "datatype": "auto-detect",
        "broker": "f0e2d1c0.92bb3",
        "nl": false,
        "rap": false,
        "inputs": 0,
        "x": 605,
        "y": 260,
        "wires": [
            [
                "8a2a0c44e1e64c1f"
            ]
        ],
        "l": false
    },
    {
        "id": "7cdc66cb731a791b",
        "type": "mqtt in",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00002/message",
        "topic": "GWY00002/message",
        "qos": "2",
        "datatype": "auto-detect",
        "broker": "dc9c4c6e985986ab",
        "nl": false,
        "rap": false,
        "inputs": 0,
        "x": 605,
        "y": 300,
        "wires": [
            [
                "3a2e111abd6339ad"
            ]
        ],
        "l": false
    },
    {
        "id": "83308769e0176daa",
        "type": "mqtt in",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00003/message",
        "topic": "GWY00003/message",
        "qos": "2",
        "datatype": "auto-detect",
        "broker": "dc9c4c6e985986ab",
        "nl": false,
        "rap": false,
        "inputs": 0,
        "x": 605,
        "y": 340,
        "wires": [
            [
                "ecec0caadc2cc8e4"
            ]
        ],
        "l": false
    },
    {
        "id": "97cf2c580f97d3a8",
        "type": "mqtt in",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00004/message",
        "topic": "GWY00004/message",
        "qos": "2",
        "datatype": "auto-detect",
        "broker": "dc9c4c6e985986ab",
        "nl": false,
        "rap": false,
        "inputs": 0,
        "x": 605,
        "y": 380,
        "wires": [
            [
                "39f2bf2cc6e3cb64"
            ]
        ],
        "l": false
    },
    {
        "id": "8a2a0c44e1e64c1f",
        "type": "switch",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "",
        "property": "GwySerNo",
        "propertyType": "flow",
        "rules": [
            {
                "t": "cont",
                "v": "GWY00001",
                "vt": "str"
            }
        ],
        "checkall": "true",
        "repair": false,
        "outputs": 1,
        "x": 655,
        "y": 260,
        "wires": [
            [
                "a3943b3ace12ead9",
                "c1593d5e1ceb29bf"
            ]
        ],
        "l": false
    },
    {
        "id": "3a2e111abd6339ad",
        "type": "switch",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "",
        "property": "GwySerNo",
        "propertyType": "flow",
        "rules": [
            {
                "t": "cont",
                "v": "GWY00002",
                "vt": "str"
            }
        ],
        "checkall": "true",
        "repair": false,
        "outputs": 1,
        "x": 655,
        "y": 300,
        "wires": [
            [
                "c8164a9ebca09224",
                "c08974d04257dbf2"
            ]
        ],
        "l": false
    },
    {
        "id": "ecec0caadc2cc8e4",
        "type": "switch",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "",
        "property": "GwySerNo",
        "propertyType": "flow",
        "rules": [
            {
                "t": "cont",
                "v": "GWY00003",
                "vt": "str"
            }
        ],
        "checkall": "true",
        "repair": false,
        "outputs": 1,
        "x": 655,
        "y": 340,
        "wires": [
            [
                "06c8087ea218b976",
                "b609244a96d993da"
            ]
        ],
        "l": false
    },
    {
        "id": "39f2bf2cc6e3cb64",
        "type": "switch",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "",
        "property": "GwySerNo",
        "propertyType": "flow",
        "rules": [
            {
                "t": "cont",
                "v": "GWY00004",
                "vt": "str"
            }
        ],
        "checkall": "true",
        "repair": false,
        "outputs": 1,
        "x": 655,
        "y": 380,
        "wires": [
            [
                "23ad1d166b2b7e88",
                "5a0355fe38be07c8"
            ]
        ],
        "l": false
    },
    {
        "id": "c1593d5e1ceb29bf",
        "type": "debug",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00001_DEBUG",
        "active": true,
        "tosidebar": true,
        "console": true,
        "tostatus": false,
        "complete": "payload",
        "targetType": "msg",
        "statusVal": "",
        "statusType": "auto",
        "x": 755,
        "y": 260,
        "wires": [],
        "l": false
    },
    {
        "id": "c08974d04257dbf2",
        "type": "debug",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00002_DEBUG",
        "active": true,
        "tosidebar": true,
        "console": true,
        "tostatus": false,
        "complete": "payload",
        "targetType": "msg",
        "statusVal": "",
        "statusType": "auto",
        "x": 755,
        "y": 300,
        "wires": [],
        "l": false
    },
    {
        "id": "b609244a96d993da",
        "type": "debug",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00003_DEBUG",
        "active": true,
        "tosidebar": true,
        "console": true,
        "tostatus": false,
        "complete": "payload",
        "targetType": "msg",
        "statusVal": "",
        "statusType": "auto",
        "x": 755,
        "y": 340,
        "wires": [],
        "l": false
    },
    {
        "id": "5a0355fe38be07c8",
        "type": "debug",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00004_DEBUG",
        "active": true,
        "tosidebar": true,
        "console": true,
        "tostatus": false,
        "complete": "payload",
        "targetType": "msg",
        "statusVal": "",
        "statusType": "auto",
        "x": 755,
        "y": 380,
        "wires": [],
        "l": false
    },
    {
        "id": "751e1553c639b344",
        "type": "link in",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00001_CMD",
        "links": [
            "053ce1b0d0d3214d"
        ],
        "x": 555,
        "y": 260,
        "wires": [
            [
                "a3943b3ace12ead9",
                "c1593d5e1ceb29bf"
            ]
        ]
    },
    {
        "id": "e13960097ca55baa",
        "type": "link in",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00002_CMD",
        "links": [
            "5a2988701c22d1b7"
        ],
        "x": 555,
        "y": 300,
        "wires": [
            [
                "c8164a9ebca09224",
                "c08974d04257dbf2"
            ]
        ]
    },
    {
        "id": "6be6ca25b09c9e73",
        "type": "link in",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00003_CMD",
        "links": [
            "b34cdd7048ec6797"
        ],
        "x": 555,
        "y": 340,
        "wires": [
            [
                "06c8087ea218b976",
                "b609244a96d993da"
            ]
        ]
    },
    {
        "id": "5cfe864119145b75",
        "type": "link in",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00004_CMD",
        "links": [
            "685d2a8af0eb6b32"
        ],
        "x": 555,
        "y": 380,
        "wires": [
            [
                "23ad1d166b2b7e88",
                "5a0355fe38be07c8"
            ]
        ]
    },
    {
        "id": "03d8bca961e516f9",
        "type": "trigger",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "AC Control",
        "op1": "ac_control",
        "op2": "",
        "op1type": "flow",
        "op2type": "nul",
        "duration": "250",
        "extend": false,
        "overrideDelay": false,
        "units": "ms",
        "reset": "",
        "bytopic": "topic",
        "topic": "topic",
        "outputs": 1,
        "x": 245,
        "y": 360,
        "wires": [
            []
        ],
        "l": false
    },
    {
        "id": "1e825eb98754dcc6",
        "type": "link in",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "link in 4",
        "links": [
            "fdce0da76334e6cd",
            "7c7fb17ab8f7ae3c",
            "712b0dd90ac477ac",
            "1552c5129f19d08c",
            "502ede3dd7559b10",
            "1d93cfe13f3deb04",
            "f932dd4a2d51ff77",
            "f8a246f18472e35a",
            "6ebcae263ab95141",
            "2d5f779104f92712",
            "20b3b596f0475a22"
        ],
        "x": 195,
        "y": 360,
        "wires": [
            [
                "03d8bca961e516f9"
            ]
        ]
    },
    {
        "id": "a1834883d3b51665",
        "type": "link in",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "link in 5",
        "links": [
            "4bdffcde7ad70484",
            "9ff792a9284b08ea"
        ],
        "x": 245,
        "y": 280,
        "wires": [
            [
                "e33e844ed5bfa869"
            ]
        ]
    },
    {
        "id": "30f9d0ca4699e562",
        "type": "link in",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "link in 6",
        "links": [
            "19f9d8acae892860"
        ],
        "x": 195,
        "y": 320,
        "wires": [
            [
                "8a12f284de75a643"
            ]
        ]
    },
    {
        "id": "8a12f284de75a643",
        "type": "trigger",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "HB Pub Conf",
        "op1": "pub_conf",
        "op2": "",
        "op1type": "flow",
        "op2type": "nul",
        "duration": "250",
        "extend": false,
        "overrideDelay": false,
        "units": "ms",
        "reset": "",
        "bytopic": "topic",
        "topic": "topic",
        "outputs": 1,
        "x": 245,
        "y": 320,
        "wires": [
            [
                "e33e844ed5bfa869"
            ]
        ],
        "l": false
    },
    {
        "id": "e33e844ed5bfa869",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "15bb5fa223da72c7",
        "name": "Json Injector",
        "func": "const json_packet_enum = Object.freeze({\n    /* GWY PACKETS */\n    GWY_REG_PACKET: 0,\n    GWY_CONF_ACK: 1,\n    GWY_UNREG_PACKET: 2,\n    GWY_AC_CONTROL_PACKET: 3,\n    GWY_MANUAL_AC_CONTROL_ACK: 4,\n    GWY_RECONF_PACKET: 5,\n    GWY_HEARTBEAT_ACK: 6,\n    GWY_HEARTBEAT_PUB_CONF_PACKET: 7,\n    GWY_TEACHING_MODE: 8,\n    GWY_TEACHING_MODE_CMD_SELECTION: 9,\n    GWY_DEBUG_INFO_PACKET: 10,\n\n    /* NODE PACKETS */\n    NODE_PROV_PACKET: 100,\n    NODE_CONF_ACK: 101,\n    NODE_UNPROV_PACKET: 102,\n    NODE_AC_CONTROL_PACKET: 103,\n    NODE_MANUAL_AC_CONTROL_ACK_PACKET: 104,\n    NODE_RECONF_PACKET: 105,\n    NODE_HEARTBEAT_ACK: 106,\n    NODE_HEARTBEAT_PUB_CONF_PACKET: 107,\n    NODE_TEACHING_MODE: 108,\n    NODE_TEACHING_MODE_CMD_SELECTION: 109,\n    NODE_DEBUG_INFO_PACKET: 110,\n\n    /* MISC PACKETS */\n    UNKNOWN_PACKET: 9999\n});\n\nlet randNum = Math.floor((Math.random() * 65535) + 1);\n\nlet Gwybasejson = {\n    \"JsonPacketID\": msg.payload,\n    \"MsgSeqNo\": randNum,\n};\n\nlet Nodebasejson = {\n    \"JsonPacketID\": msg.payload,\n    \"MsgSeqNo\": randNum,\n    \"ElementAddr\": flow.get(\"ElementAddr\"),\n}\n\nlet json;\nswitch (msg.payload) {\n    case json_packet_enum.GWY_REG_PACKET:\n        json = {\n            \"Location\": flow.get(\"Location\")\n        };\n        msg = { payload: Object.assign({}, Gwybasejson, json) };\n        break;\n\n    case json_packet_enum.GWY_UNREG_PACKET:\n        msg = { payload: Object.assign({}, Gwybasejson) };\n        break;\n\n    case json_packet_enum.GWY_AC_CONTROL_PACKET:\n        json = {\n            \"Power\": flow.get(\"Power\"),\n            \"Temperature\": flow.get(\"Temperature\"),\n            \"FanSpeed\": flow.get(\"FanSpeed\"),\n            \"Mode\": flow.get(\"Mode\"),\n            \"SwingH\": flow.get(\"SwingH\"),\n            \"SwingV\": flow.get(\"SwingV\"),\n            \"Locking\": flow.get(\"Locking\"),\n            \"OnTimer\": flow.get(\"OnTimer\"),\n            \"OffTimer\": flow.get(\"OffTimer\"),\n            \"TempLockUpLimit\": flow.get(\"TempLockUpLimit\"),\n            \"TempLockLowLimit\": flow.get(\"TempLockLowLimit\")\n        };\n        msg = { payload: Object.assign({}, Gwybasejson, json) };\n        break;\n\n    case json_packet_enum.GWY_RECONF_PACKET:\n        msg = { payload: Object.assign({}, Gwybasejson) };\n        break;\n\n    case json_packet_enum.GWY_TEACHING_MODE:\n        json = {\n            \"TeachingStart\": flow.get(\"TeachingStart\"),\n            \"StartingTemperature\": flow.get(\"StartingTemperature\"),\n            \"EndingTemperature\": flow.get(\"EndingTemperature\"),\n        };\n        msg = { payload: Object.assign({}, Gwybasejson, json) };\n        break;\n\n    case json_packet_enum.GWY_TEACHING_MODE_CMD_SELECTION:\n        json = {\n            \"Power\": flow.get(\"Power\"),\n            \"Temperature\": flow.get(\"Temperature\")\n        };\n        msg = { payload: Object.assign({}, Gwybasejson, json) };\n        break;\n\n    case json_packet_enum.GWY_HEARTBEAT_PUB_CONF_PACKET:\n        json = {\n            \"PublishPeriodSec\": flow.get(\"GwyPublishPeriod\")\n        };\n        msg = { payload: Object.assign({}, Gwybasejson, json) };\n        break;\n\n    case json_packet_enum.GWY_DEBUG_INFO_PACKET:\n        json = {\n            \"ResetDevice\": flow.get(\"ResetDevice\"),\n            \"RestartDevice\": flow.get(\"RestartDevice\")\n        };\n        msg = { payload: Object.assign({}, Gwybasejson, json) };\n        break;\n\n    case json_packet_enum.NODE_UNPROV_PACKET:\n        msg = { payload: Object.assign({}, Nodebasejson) };\n        break;\n\n    case json_packet_enum.NODE_AC_CONTROL_PACKET:\n        json = {\n            \"Power\": flow.get(\"Power\"),\n            \"Temperature\": flow.get(\"Temperature\"),\n            \"FanSpeed\": flow.get(\"FanSpeed\"),\n            \"Mode\": flow.get(\"Mode\"),\n            \"SwingH\": flow.get(\"SwingH\"),\n            \"SwingV\": flow.get(\"SwingV\"),\n            \"Locking\": flow.get(\"Locking\"),\n            \"OnTimer\": flow.get(\"OnTimer\"),\n            \"OffTimer\": flow.get(\"OffTimer\"),\n            \"TempLockUpLimit\": flow.get(\"TempLockUpLimit\"),\n            \"TempLockLowLimit\": flow.get(\"TempLockLowLimit\")\n        };\n        msg = { payload: Object.assign({}, Nodebasejson, json) };\n        break;\n\n    case json_packet_enum.NODE_RECONF_PACKET:\n        msg = { payload: Object.assign({}, Nodebasejson) };\n        break;\n\n    case json_packet_enum.NODE_HEARTBEAT_PUB_CONF_PACKET:\n        json = {\n            \"PublishPeriodSec\": flow.get(\"NodePublishPeriod\")\n        };\n        msg = { payload: Object.assign({}, Nodebasejson, json) };\n        break;\n\n    case json_packet_enum.NODE_TEACHING_MODE:\n        json = {\n            \"TeachingStart\": flow.get(\"TeachingStart\"),\n            \"StartingTemperature\": flow.get(\"StartingTemperature\"),\n            \"EndingTemperature\": flow.get(\"EndingTemperature\"),\n        };\n        msg = { payload: Object.assign({}, Nodebasejson, json) };\n        break;\n\n    case json_packet_enum.NODE_TEACHING_MODE_CMD_SELECTION:\n        json = {\n            \"Power\": flow.get(\"Power\"),\n            \"Temperature\": flow.get(\"Temperature\")\n        };\n        msg = { payload: Object.assign({}, Nodebasejson, json) };\n        break;\n\n    case json_packet_enum.NODE_DEBUG_INFO_PACKET:\n        json = {\n            \"ResetDevice\": flow.get(\"ResetDevice\"),\n            \"RestartDevice\": flow.get(\"RestartDevice\")\n        };\n        msg = { payload: Object.assign({}, Nodebasejson, json) };\n        break;\n}\nreturn msg;",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 305,
        "y": 320,
        "wires": [
            [
                "ba803b0afe363686"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "fec9c17deae6082b",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "cb5cb5d3935e7873",
        "name": "GWY00004",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "GWY00004",
        "payloadType": "str",
        "x": 1401,
        "y": 601,
        "wires": [
            [
                "51d756f0287c566a"
            ]
        ]
    },
    {
        "id": "4124febb2ccdbec4",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "TempLockUpLimit",
        "func": "flow.set(\"StartingTemperature\",msg.payload);",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 875,
        "y": 1520,
        "wires": [
            []
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "eb334519f509740d",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "18",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "18",
        "payloadType": "num",
        "x": 280,
        "y": 1480,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "0d877d3ba67432ad",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "19",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "19",
        "payloadType": "num",
        "x": 400,
        "y": 1480,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "4c8c8a7866ec297b",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "20",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "20",
        "payloadType": "num",
        "x": 520,
        "y": 1480,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "db596926b6473797",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "21",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "21",
        "payloadType": "num",
        "x": 640,
        "y": 1480,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "b19fc4fc23e254a7",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "22",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "22",
        "payloadType": "num",
        "x": 760,
        "y": 1480,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "70e5f5d1fbdebafe",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "23",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "23",
        "payloadType": "num",
        "x": 280,
        "y": 1520,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "eb8fdb9366c0cbfd",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "24",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "24",
        "payloadType": "num",
        "x": 400,
        "y": 1520,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "751d9a7ce447ef95",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "25",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "25",
        "payloadType": "num",
        "x": 520,
        "y": 1520,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "812388750930990d",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "26",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "26",
        "payloadType": "num",
        "x": 640,
        "y": 1520,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "f6ae080f498b5ae1",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "27",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "27",
        "payloadType": "num",
        "x": 760,
        "y": 1520,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "1905b3e90ec791e9",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "28",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "28",
        "payloadType": "num",
        "x": 280,
        "y": 1560,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "d31aed671d48287a",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "29",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "29",
        "payloadType": "num",
        "x": 400,
        "y": 1560,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "7b2003f18d98edb2",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "30",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "30",
        "payloadType": "num",
        "x": 520,
        "y": 1560,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "395ac3fa2c7726a0",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "31",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "31",
        "payloadType": "num",
        "x": 640,
        "y": 1560,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "149ef0b6108c288e",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "38c6e51186b314d2",
        "name": "32",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "32",
        "payloadType": "num",
        "x": 760,
        "y": 1560,
        "wires": [
            [
                "4124febb2ccdbec4"
            ]
        ]
    },
    {
        "id": "a0ea49b49acea139",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "TempLockUpLimit",
        "func": "flow.set(\"EndingTemperature\",msg.payload);",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 875,
        "y": 1720,
        "wires": [
            []
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "a62f660515e074e0",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "18",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "18",
        "payloadType": "num",
        "x": 280,
        "y": 1680,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "f81cbaaad3b34539",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "19",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "19",
        "payloadType": "num",
        "x": 400,
        "y": 1680,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "66d3e34b31a12241",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "20",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "20",
        "payloadType": "num",
        "x": 520,
        "y": 1680,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "2c01ae7b2748d4be",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "21",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "21",
        "payloadType": "num",
        "x": 640,
        "y": 1680,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "01e345e30338f447",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "22",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "22",
        "payloadType": "num",
        "x": 760,
        "y": 1680,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "614cda551775c11d",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "23",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "23",
        "payloadType": "num",
        "x": 280,
        "y": 1720,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "92e219519ab93f1e",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "24",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "24",
        "payloadType": "num",
        "x": 400,
        "y": 1720,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "710f21a15ba926aa",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "25",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "25",
        "payloadType": "num",
        "x": 520,
        "y": 1720,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "5c6ceb71bc82058e",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "26",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "26",
        "payloadType": "num",
        "x": 640,
        "y": 1720,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "d910d67059cd97e9",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "27",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "27",
        "payloadType": "num",
        "x": 760,
        "y": 1720,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "3b1150787b4e9215",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "28",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "28",
        "payloadType": "num",
        "x": 280,
        "y": 1760,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "6a4e980fe59862c2",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "29",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "29",
        "payloadType": "num",
        "x": 400,
        "y": 1760,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "b7cfe130556c401d",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "30",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "30",
        "payloadType": "num",
        "x": 520,
        "y": 1760,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "9a9ff2bc08a39941",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "31",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "31",
        "payloadType": "num",
        "x": 640,
        "y": 1760,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "629ddef91f242a15",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "a2e72fc4f574a532",
        "name": "32",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "32",
        "payloadType": "num",
        "x": 760,
        "y": 1760,
        "wires": [
            [
                "a0ea49b49acea139"
            ]
        ]
    },
    {
        "id": "dbb315e0d2b26759",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "9478a12968e0865e",
        "name": "Locking",
        "func": "flow.set(\"TeachingStart\",msg.payload);\n\nif(flow.get(\"Device\")==1)\n{\n    msg.payload = 108;\n    return msg;\n}\nelse\n{\n    msg.payload = 8;\n    return msg;\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1205,
        "y": 1460,
        "wires": [
            []
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "682989149c9de46c",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "9478a12968e0865e",
        "name": "Teaching Start",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1",
        "payloadType": "num",
        "x": 1090,
        "y": 1440,
        "wires": [
            [
                "dbb315e0d2b26759"
            ]
        ]
    },
    {
        "id": "c6e19313557c32b3",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "9478a12968e0865e",
        "name": "Teaching End",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "0",
        "payloadType": "num",
        "x": 1080,
        "y": 1480,
        "wires": [
            [
                "dbb315e0d2b26759"
            ]
        ]
    },
    {
        "id": "4eb1fc758a9ae0ab",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "Teaching Mode Cmd Selection",
        "props": [
            {
                "p": "payload"
            },
            {
                "p": "topic",
                "vt": "str"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "9",
        "payloadType": "num",
        "x": 320,
        "y": 720,
        "wires": [
            [
                "8859639b97d711e2"
            ]
        ]
    },
    {
        "id": "8859639b97d711e2",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "93250921e6d76682",
        "name": "GwyPublishPeriod",
        "func": "if(flow.get(\"Device\")==1)\n{\n    msg.payload = 109;\n    return msg;\n}\nelse\n{\n    msg.payload = 9;\n    return msg;\n}",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 495,
        "y": 720,
        "wires": [
            [
                "4bdffcde7ad70484"
            ]
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "3fc78c676c50424d",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "0e9f32d208ba4b2e",
        "name": "Locking",
        "func": "flow.set(\"ResetDevice\",msg.payload);\n",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1515,
        "y": 760,
        "wires": [
            []
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "c6e0cf1e0aa4a4d6",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "0e9f32d208ba4b2e",
        "name": "Reset ON",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1",
        "payloadType": "num",
        "x": 1400,
        "y": 740,
        "wires": [
            [
                "3fc78c676c50424d"
            ]
        ]
    },
    {
        "id": "1a77cd19feb8ef4e",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "0e9f32d208ba4b2e",
        "name": "Reset OFF",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "0",
        "payloadType": "num",
        "x": 1410,
        "y": 780,
        "wires": [
            [
                "3fc78c676c50424d"
            ]
        ]
    },
    {
        "id": "25e29728a45fcd87",
        "type": "function",
        "z": "9c54f3c8c089fa45",
        "g": "88753f5d31542be8",
        "name": "Locking",
        "func": "flow.set(\"RestartDevice\",msg.payload);\n",
        "outputs": 1,
        "timeout": "",
        "noerr": 0,
        "initialize": "",
        "finalize": "",
        "libs": [],
        "x": 1515,
        "y": 900,
        "wires": [
            []
        ],
        "inputLabels": [
            "Json Packet ID"
        ],
        "outputLabels": [
            "JSON"
        ],
        "l": false,
        "info": "Creates Dynamic JSON as per User Input Values"
    },
    {
        "id": "1b29124113a3f7aa",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "88753f5d31542be8",
        "name": "Restart ON",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": false,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "1",
        "payloadType": "num",
        "x": 1400,
        "y": 880,
        "wires": [
            [
                "25e29728a45fcd87"
            ]
        ]
    },
    {
        "id": "70f0814af507b85d",
        "type": "inject",
        "z": "9c54f3c8c089fa45",
        "g": "88753f5d31542be8",
        "name": "Restart OFF",
        "props": [
            {
                "p": "payload"
            }
        ],
        "repeat": "",
        "crontab": "",
        "once": true,
        "onceDelay": 0.1,
        "topic": "",
        "payload": "0",
        "payloadType": "num",
        "x": 1410,
        "y": 920,
        "wires": [
            [
                "25e29728a45fcd87"
            ]
        ]
    },
    {
        "id": "c8164a9ebca09224",
        "type": "flogger",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00002_LOGGER",
        "logfile": "GWY00002_LOG",
        "inputchoice": "object",
        "inputobject": "payload",
        "inputobjectType": "msg",
        "inputmoustache": "Recieved payload {{payload}} and topic {{topic}}",
        "loglevel": "INFO",
        "logconfig": "3cbf671b3ee06b85",
        "sendpane": "",
        "x": 705,
        "y": 300,
        "wires": [
            []
        ],
        "l": false
    },
    {
        "id": "a3943b3ace12ead9",
        "type": "flogger",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00001_LOGGER",
        "logfile": "GWY00001_LOG",
        "inputchoice": "object",
        "inputobject": "payload",
        "inputobjectType": "msg",
        "inputmoustache": "Recieved payload {{payload}} and topic {{topic}}",
        "loglevel": "INFO",
        "logconfig": "3cbf671b3ee06b85",
        "sendpane": "",
        "x": 705,
        "y": 260,
        "wires": [
            []
        ],
        "l": false
    },
    {
        "id": "06c8087ea218b976",
        "type": "flogger",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00003_LOGGER",
        "logfile": "GWY00003_LOG",
        "inputchoice": "object",
        "inputobject": "payload",
        "inputobjectType": "msg",
        "inputmoustache": "Recieved payload {{payload}} and topic {{topic}}",
        "loglevel": "INFO",
        "logconfig": "3cbf671b3ee06b85",
        "sendpane": "",
        "x": 705,
        "y": 340,
        "wires": [
            []
        ],
        "l": false
    },
    {
        "id": "23ad1d166b2b7e88",
        "type": "flogger",
        "z": "9c54f3c8c089fa45",
        "g": "8453c3c42afc8ad4",
        "name": "GWY00004_LOGGER",
        "logfile": "GWY00004_LOG",
        "inputchoice": "object",
        "inputobject": "payload",
        "inputobjectType": "msg",
        "inputmoustache": "Recieved payload {{payload}} and topic {{topic}}",
        "loglevel": "INFO",
        "logconfig": "3cbf671b3ee06b85",
        "sendpane": "",
        "x": 705,
        "y": 380,
        "wires": [
            []
        ],
        "l": false
    },
    {
        "id": "f0e2d1c0.92bb3",
        "type": "mqtt-broker",
        "name": "Unimation",
        "broker": "3.7.8.183",
        "port": "1883",
        "clientid": "",
        "autoConnect": true,
        "usetls": false,
        "compatmode": false,
        "protocolVersion": "4",
        "keepalive": "60",
        "cleansession": true,
        "autoUnsubscribe": true,
        "birthTopic": "",
        "birthQos": "0",
        "birthPayload": "",
        "birthMsg": {},
        "closeTopic": "",
        "closeQos": "0",
        "closePayload": "",
        "closeMsg": {},
        "willTopic": "",
        "willQos": "0",
        "willPayload": "",
        "willMsg": {},
        "userProps": "",
        "sessionExpiry": ""
    },
    {
        "id": "dc9c4c6e985986ab",
        "type": "mqtt-broker",
        "name": "QMAX",
        "broker": "54.215.188.103",
        "port": "1883",
        "clientid": "",
        "autoConnect": true,
        "usetls": false,
        "protocolVersion": "4",
        "keepalive": "10",
        "cleansession": true,
        "autoUnsubscribe": true,
        "birthTopic": "",
        "birthQos": "0",
        "birthRetain": "false",
        "birthPayload": "",
        "birthMsg": {},
        "closeTopic": "",
        "closeQos": "0",
        "closeRetain": "false",
        "closePayload": "",
        "closeMsg": {},
        "willTopic": "",
        "willQos": "2",
        "willRetain": "false",
        "willPayload": "",
        "willMsg": {},
        "userProps": "",
        "sessionExpiry": ""
    },
    {
        "id": "3cbf671b3ee06b85",
        "type": "config-log",
        "logname": "Config 1",
        "logdir": "/Users/Computer/Desktop/",
        "stamp": "local",
        "logstyle": "plain",
        "logrotate": false,
        "logcompress": false,
        "logrotatecount": "5",
        "logsize": "1000"
    }
]