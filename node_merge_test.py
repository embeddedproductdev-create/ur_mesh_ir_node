...
Merges bootloader + partition table + one N-prefix serial-number bin (or all
of them) + app into a single output folder. No versioning, no git checks.
Always overwrites the previous test output so you can iterate quickly.
...
Serial number format: N-prefix + 5-digit zero-padded number (e.g. N00001).
Generated by SerNoGen.py using --deviceType NODE (first char = N).

Usage:
    python node_merge_test.py           # merge ALL N*.bin serial bins found
    python node_merge_test.py N00001    # merge just one device, by name

Prerequisites:
    1. Build the node firmware (IS_GWY=0) in Espressif IDE.
    2. Generate NVS serial bins:
           python SerNoGen.py --deviceType NODE --start_serial 0 --end_serial 100
    3. Run this script to produce a test merged binary.
    4. Flash and verify on hardware before proceeding to node_merge_release.py.
...
def main():
    ...
    all_serial_bins = sorted(
        ...
        if f.lower().endswith(".bin") and os.path.splitext(f)[0].upper().startswith("N")
    )
