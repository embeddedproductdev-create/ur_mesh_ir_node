...
namespace irutils
{
  ...
  String modelToStr(const decode_type_t protocol, const int16_t model)
  {
    ...
    case decode_type_t::MIRAGE:
      switch (model)
      {
      case mirage_ac_remote_model_t::KKG9AC1:
        return kKkg9ac1Str;
    ...
    case decode_type_t::SHARP_AC:
      switch (model)
      {
      case sharp_ac_remote_model_t::A907:
        return kA907Str;
    ...
    case decode_type_t::TCL112AC:
      switch (model)
      {
      case tcl_ac_remote_model_t::TAC09CHSD:
        return kTac09chsdStr;
      case tcl_ac_remote_model_t::GZ055BE1:
        return kGz055be1Str;
      default:
        return kUnknownStr;
      }
    ...
    case decode_type_t::ARGO:
      switch (model)
      {
      case argo_ac_remote_model_t::SAC_WREM2:
        return kArgoWrem2Str;
      case argo_ac_remote_model_t::SAC_WREM3:
        return kArgoWrem3Str;
      default:
        return kUnknownStr;
      }
      break;
  ...
  {
    String result = addIntToString(degrees, (isSensorTemp) ? kSensorTempStr : kTempStr, precomma);
    result += celsius ? 'C' : 'F';
    return result;
  }
                              ...
                              const bool precomma, const bool isSensorTemp)
  {
    ...
    result += addIntToString(degrees, (isSensorTemp) ? kSensorTempStr : kTempStr, precomma);
    // Is it a half degree?
    if (((uint16_t)(2 * degrees)) & 1)
      result += F(".5");
    result += celsius ? 'C' : 'F';
    return result;
  }
...
#if UNIT_TEST
      return String(kThreeLetterDayOfWeekStr).substr((day_of_week + offset) * 3, 3);
#else  // UNIT_TEST
      return String(kThreeLetterDayOfWeekStr).substring((day_of_week + offset) * 3, (day_of_week + offset) * 3 + 3);
#endif // UNIT_TEST
    else
      return kUnknownStr;
  }
  ...
  String addFanToString_daikin200(const uint8_t speed, const uint8_t automatic, const uint8_t min,
                                  const uint8_t low, const uint8_t med, const uint8_t high, const uint8_t max)
  {
    String result = "";
    result.reserve(21); // ", Fan: NNN (UNKNOWN)"
    result += addIntToString(speed, kFanStr);
  ...
  String msToString(uint32_t const msecs)
  {
    ...
    if (days)
      result += uint64ToString(days) + ' ' + String((days > 1) ? kDaysStr : kDayStr);
    if (hours)
    {
      if (result.length())
        result += ' ';
      result += uint64ToString(hours) + ' ' + String((hours > 1) ? kHoursStr : kHourStr);
    }
    if (minutes)
    {
      if (result.length())
        result += ' ';
      result += uint64ToString(minutes) + ' ' + String((minutes > 1) ? kMinutesStr : kMinuteStr);
    }
    if (seconds)
    {
      if (result.length())
        result += ' ';
      result += uint64ToString(seconds) + ' ' + String((seconds > 1) ? kSecondsStr : kSecondStr);
    }
    return result;
  }
                    ...
                    const uint8_t init, const bool byteonly)
  {
    ...
    for (uint8_t i = 0; i < nrofbytes; i++, copy >>= 8)
      sum += (copy & 0xFF);
