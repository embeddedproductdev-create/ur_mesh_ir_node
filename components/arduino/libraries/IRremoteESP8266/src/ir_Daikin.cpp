                        ...
                        const uint16_t repeat)
{
  ...
  for (uint16_t r = 0; r <= repeat; r++)
  {
    ...
    { // We are using the newer/more correct size.
      sendGeneric(kDaikinHdrMark, kDaikinHdrSpace, kDaikinBitMark,
                  kDaikinOneSpace, kDaikinBitMark, kDaikinZeroSpace,
                  kDaikinBitMark, kDaikinZeroSpace + kDaikinGap,
...
{
  uint8_t offset = 0;
  if (length == kDaikinStateLengthShort)
  { // Handle the "short" length case.
...
{
  for (uint8_t i = 0; i < kDaikin200StateLength; i++)
    _.raw[i] = 0x00;
  _.raw[0] = 0x11;
  _.raw[1] = 0xDA;
  _.raw[2] = 0x17;
...
String IRDaikin200::toString(void) const
{
  ...
  result.reserve(120); // Reserve some heap for the string to reduce fragging.
  result += addBoolToString(_.Power, kPowerStr, false);
  result += addModeToString(_.Mode, kDaikin200Auto, kDaikin200Cool, kDaikin200Heat,
                            kDaikin200Dry, kDaikin200Fan);
  result += addTempToString(_.Temp + 9);
  result += irutils::addFanToString_daikin200(_.Fan, kDaikin200FanAuto, kDaikin200FanMin,
                                     kDaikin200FanLow, kDaikin200FanMed,
                                     kDaikin200FanHigh, kDaikin200FanMax);
  return result;
}
