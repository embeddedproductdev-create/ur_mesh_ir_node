...
static void USE_IRAM_ATTR gpio_intr() {
  ...
  timerWrite(timer, 0);
  // timerAlarmEnable(timer);//commentedhere
...
void IRrecv::enableIRIn(const bool pullup) {
  ...
  // timerAlarmWrite(timer, MS_TO_USEC(params.timeout), ONCE);//commmentedhere
  timerAlarm(timer, MS_TO_USEC(params.timeout), true, false);
...
#endif  // ESP8266
#if defined(ESP32)
  // timerAlarmDisable(timer);//commentedhere
