...
const uint16_t kIrLed = 2;  // ESP32 GPIO pin to use. On BlueRC shield: 7 (D14).
...
void sendIRCode(char Button) {
  ...
  if(Button == 'u')
  {
    ...
    irsend.sendNEC(0x009f9f00, 32);
    irsend.sendNEC(0x00a22a00, 32);
  }
  else
  {
    Serial.println("Sending Volume Down");
//    irsend.sendRC5(0xA2, 12, 0);
    irsend.sendNEC(0x009f9f0000a2, 32);
    delay(1000);
  }
  delay(20);
}
...
//  sendIRCode('a');
  ...
  irsend.sendNEC(0x00f9ba45, 32);//45
  delay(1000);
  ...
  delay(1000);
  Serial.println("Sending ON/OFF button command ...");
